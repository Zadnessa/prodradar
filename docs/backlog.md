...
1. Семантическое ядро PM-заголовков (Stream 3A)
2. Трёхзонная фильтрация + рефакторинг main.py (Stream 3A)
3. Отраслевая классификация (Stream 3B)
4. UX-блок: онбординг, настройки, виртуальная эмпатия, пагинация, кнопка «Получить вакансии», ReplyKeyboardMarkup, приветственное сообщение (Stream 3C)
5. Аналитика: таблица user_events, логирование событий, скрипт /broadcast (Stream 3D)
6. Квик-вины: «проджект» в стоп-слова, Chrome UA, debug.yml (Stream 3F)
7. Код-ревью от внешнего аудитора (Stream 3F)
8. Ревизия текстов бота (Stream 3C) — строго последняя задача
9. Предрелизный аудит (Wave 4)
...
- [ ] Отрасли только в /settings advanced, не в онбординге. Онбординг: grade -> city -> work_format -> confirm -> disclaimer. После онбординга — подсветка: «Хочешь настроить компании по отраслям? Жми /settings -> Расширенные»
- [ ] Пагинация выдачи: текущая проблема — если пользователь не нажимает «ещё 10» до конца, непоказанные вакансии остаются непомеченными как доставленные и всплывают как «новые» в следующем прогоне, хотя они не новые. Решение определяется по результатам виртуальной эмпатии — варианты включают: (а) помечать все подходящие вакансии доставленными сразу при scheduled-выдаче, показывать порциями, кнопка «ещё» раскрывает уже помеченные; (б) дайджест со списком и раскрытием по клику; (в) другие варианты из CJM-прогона.
...
- [ ] Виртуальная эмпатия по формату выдачи: подготовить варианты решений (дайджест, порционная выдача, выдача сразу целиком) и провести CJM-прогон по двум персонам (активный соискатель, пассивный наблюдатель) для выбора оптимального формата. Выполняется до начала разработки пагинации.

После релиза:
- [ ] Полная ревизия текстов бота (это ФИНАЛЬНАЯ задача перед релизом — выполняется когда всё остальное уже работает; пройти каждый экран, убрать технические термины, привести тон к «помощник, не система»)
...
- [ ] Подготовить цели и промпт для код-ревью от внешнего аудитора (thinking-LLM). Выполняется после завершения всех задач до релиза, перед ревизией текстов. Цели: проверка хардкода, контракты парсеров, необработанные исключения, соответствие main.py и AGENTS.md, рассинхрон документации и кода.
