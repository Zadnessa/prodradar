...
- ## 2026-03-15: Новые правила

### Правило работы с supabase-py
- Метод `.not_in()` НЕ существует в supabase-py/postgrest-py. Использовать `.not_.in_("column", list)`.
- При любых сомнениях в API supabase-py — проверять через https://supabase.com/docs/reference/python/not

### Правило верификации AI-аудитов
- Любые утверждения AI-аудитора о состоянии кода ОБЯЗАТЕЛЬНО проверяются через raw GitHub URLs перед принятием решений.
- Аудитор может читать кэшированную или устаревшую версию файла.

### Текущие UX-решения (не реализованы, но приняты)
- /start для существующего пользователя должен показывать хаб выбора, а не сбрасывать всё.
- /start должен всегда снимать paused=true.
- Кнопка "Получить вакансии" в /settings показывается всегда, не только после save.
- После настройки фильтров — disclaimer о неполноте данных API.
- Поддержка двух форматов выдачи: карточки и дайджест (выбор пользователя).
- Кнопки на карточках: сохранить, скрыть, похожие (после стабилизации выдачи).

### Cron-рассылка
- main.py ДОЛЖЕН фильтровать ПОСЛЕ полной выборки, не до. Текущий limit=10 до фильтрации — баг.
- Целевая логика: get_undelivered_vacancies(limit=200) → filter → sort by created_at desc → [:10] → send → mark_delivered.

### Мёртвый код
- delivery/telegram.py:deliver_vacancies() — мёртвый код, удалить при первой возможности.

### Stop-текст
- get_stop_confirm() текст "Фильтры будут сброшены" не соответствует реальности. Привести в соответствие.

## Планируемые волны доработок (верхний уровень)

Wave 1 — Блокеры доверия:
- /start хаб для returning user
- /start снимает паузу
- Cron: limit после фильтрации
- "Получить вакансии" всегда в /settings
- Фильтр города: переосмысление "Другой"
- stop-текст привести к реальности

Wave 2 — Качество выдачи:
- Честная пагинация (snapshot/cursor вместо shuffle)
- Разделение zero-state на 4 сценария
- /stats: три метрики из одного пула
- Расписание рассылок в текстах
- Disclaimer о null-полях

Wave 3 — UX-улучшения:
- Два формата выдачи (карточки / дайджест)
- Кнопки на карточках
- Удаление мёртвого кода
- Единая логика доставки (webhook + cron)
