...
Текущие парсеры (16): Avito, Yandex, T-Bank, Alfa-Bank, Ozon, Sber, VK, Wildberries, Dodo, Купер, Точка, Циан, Контур, Lamoda, ДомКлик, МТС.
...
- [ ] Семантическое ядро PM-заголовков: через API hh.ru (api.hh.ru) выгрузить вакансии продакт-менеджеров по professional_role_id, собрать все уникальные варианты написания заголовков. На основе этого построить универсальный whitelist заголовков для всех компаний. Цель: расширить охват PM-вакансий в компаниях где продуктовые роли размазаны по разным категориям (МТС, Сбер). Заменит текущие per-company whitelist-ы.
