...
Текущие парсеры (14): Avito, Yandex, T-Bank, Alfa-Bank, Ozon, Sber, VK, Wildberries, Dodo, Точка, Циан, Контур, Lamoda, ДомКлик.
...
- [ ] Сделать retry для парсеров: обернуть HTTP-запросы в parse() и enrich() в retry с экспоненциальным backoff (2-3 попытки, интервал 2/4/8 сек). В админ-отчёте указывать количество попыток. Реализовать как общий декоратор или утилиту, не дублировать в каждом парсере.
- [ ] Определить конкретные названия отраслей для category (владелец продукта)
- [ ] Fetcher-абстракция: модуль fetcher.py с двумя транспортами (aiohttp для чистых API, Playwright для антибот-сайтов). Колонка fetch_method в таблице companies. Парсеры получают контент через fetcher, не ходят в сеть напрямую.
- [ ] Миграция проблемных парсеров на Playwright: Циан (убрать requests.Session + прогрев кук), СберЗдоровье (убрать buildId). Остальные парсеры не трогать.
- [ ] Аудит костылей в текущих парсерах: пройти все спецификации в api_spec.md, выявить хардкод и workaround-ы, которые решаются browser_fetch.
