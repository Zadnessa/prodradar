...
class HHZvukParser(BaseParser):
    ...
    async def enrich(self, session, vacancy):
        ...
        try:
            ...
            raw_id = vacancy_id.removeprefix("hh_zvuk_")
            if not raw_id:
                return vacancy
            ...
            logger.warning("Ошибка enrichment Звук для %s: %s", vacancy.get("id"), exc)
        finally:
            if should_sleep:
