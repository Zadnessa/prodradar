...
Текущие парсеры (23): 2ГИС, Alfa-Bank, Aviasales, Avito, Dodo, Касперский (HH), Lamoda, Ozon, Sber, T-Bank, VK, Wildberries, X5 Group (X5 Tech, Пятёрочка, Перекрёсток, Чижик, X5 Media, X5 Импорт, Много лосося — один парсер, несколько брендов через BRAND_MAP), Yandex, ДомКлик, Контур, Купер, МТС (МТС, МТС Банк, KION, Юрент, MWS.AI — один парсер, несколько брендов через BRAND_MAP), МТС Линк, СберЗдоровье, Точка, Циан, Циан (HH), Звук (HH API).
...
- [x] Playwright как второй транспорт: установить, протестировать в Colab на проблемных компаниях (СберЗдоровье, Циан), использовать для парсеров типа D. Добавить в requirements.txt. Кэшировать Chromium в GitHub Actions.
- [x] curl_cffi как третий транспорт: для сайтов с WAF, блокирующим по TLS fingerprint. Добавлен в requirements.txt. Используется в Циан (enrichment).
